...
const MenuScreen: React.FC<Props> = ({navigation}) => {
  ...
  const handleSubmitChangePassword = async () => {
    ...
    try {
      ...
      await axios.post(
        'http://192.168.16.146:3002/api/hotparts/cambiarPassword',
        {
          nomina: user.Nomina,
    ...
    useCallback(() => {
      const checkUnread = async () => {
        const count = parseInt(
          (await AsyncStorage.getItem('unreadNotificationsCount')) ?? '0',
          10,
        );
  ...
  useFocusEffect(
    useCallback(() => {
      const checkAvailability = async () => {
        ...
        try {
          const endpoint =
            user.Area === 'Produccion' ? 'Produccion' : 'calidad';
          const response = await axios.get(
            `http://192.168.16.146:3002/api/hotparts/${endpoint}`,
          );
          const data: any[] = response.data || [];

          if (user.Area === 'Produccion') {
            const hasFaltante = data.some(
              item => Number(item['Cantidad Faltante por Entregar']) > 0,
            );
            ...
            setCanEntregar(
              data.some(
                item => Number(item['Cantidad Faltante de Entregar']) > 0,
              ),
            );
            setCanReorden(
              data.some(
                item => Number(item['Cantidad Recibida de Produccion']) > 0,
              ),
            );
          }
        } catch (error) {
          console.error(
            'Error al verificar disponibilidad de Hot Parts:',
            error,
          );
        }
      };
            ...
            {user.Area === 'Calidad' && (
              <>
                <TouchableOpacity
                  style={[
                    styles.optionCard,
                    !canEntregar && styles.optionCardDisabled,
                  ]}
                  onPress={handleEntrega}
                  activeOpacity={0.85}
                ...
                <TouchableOpacity
                  style={[
                    styles.optionCard,
                    !canReorden && styles.optionCardDisabled,
                  ]}
        ...
        <View style={styles.sidebarWrapper}>
          ...
          <Animated.View
            ...
            <View style={styles.sidebarPanel}>
              <View
                style={[styles.sidebarHeader, {paddingTop: insets.top + 28}]}>
                ...
                <Text style={styles.sidebarNombre} numberOfLines={1}>
                  {user.Nombre}
                </Text>
                <Text style={styles.sidebarNominaText}>
                  Nómina {user.Nomina}
                </Text>
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}>
                  <Text style={styles.viewAllHotPartsButtonText}>
                    Ver todos los HotParts
                  </Text>
                </TouchableOpacity>
