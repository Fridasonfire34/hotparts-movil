...
type EntregaCalidadScreenRouteProp = RouteProp<
  RootStackParamList,
  'EntregaCalidad'
>;
...
const EntregaCalidadScreen: React.FC<Props> = ({route}) => {
  ...
  const handleRecibirHotPart = async () => {
    ...
    if (rowsWithQuantityOne.length > 0) {
      ...
      const cantidades = rowsWithQuantityOne.map(
        item => item['Cantidad Faltante de Entregar'],
      );
  ...
  const enviarListadoEntregaProduccion = async () => {
    ...
    try {
      await axios.post(
        'http://192.168.16.146:3002/api/hotparts/listadoEntregaCalidad',
        {
  ...
  const handleQuantityConfirm = async () => {
    ...
    if (
      !quantityToDeliver ||
      quantityToDeliver <= 0 ||
      quantityToDeliver > item['Cantidad Faltante de Entregar']
    ) {
      Alert.alert(
        'Error',
    ...
    try {
      ...
      if (verifyResponse.data.success) {
        const reciboResponse = await axios.post(
          'http://192.168.16.146:3002/api/hotparts/reciboEmbarques',
          {
            folios: foliosSeleccionados,
            nomina: nomina,
          },
              ...
              onPress: async () => {
                try {
                  ...
                  const entregaResponse = await axios.post(
                    'http://192.168.16.146:3002/api/hotparts/entregaCalidad',
                    {
                      folios: foliosSeleccionados,
                      nomina: nomina,
                      area: area,
      ...
      <TouchableOpacity
        ...
        <View style={styles.cardRow}>
          ...
          <Text style={styles.cardQty}>
            {item['Cantidad Faltante de Entregar']}
          </Text>
        </View>
      </TouchableOpacity>
    );
              ...
              <SectionList
                ...
                renderSectionHeader={({section}) => {
                  const totalPiezas = section.data.reduce(
                    (sum: number, item: any) =>
                      sum + Number(item['Cantidad Faltante de Entregar']),
                    0,
                  );
                  return (
