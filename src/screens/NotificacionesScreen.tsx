  ...
  const fetchNotificaciones = async () => {
    try {
      const response = await axios.get(
        'http://192.168.16.146:3002/api/hotparts/notificaciones',
      );
      ...
      Alert.alert(
        'Error',
        'No se pudo obtener el historial de notificaciones.',
      );
    }
  };
            ...
            {isExpanded && (
              <View style={styles.detalleContainer}>
                <Text style={styles.detalleUsuarios}>
                  Quien Entregó:{' '}
                  <Text style={styles.detalleUsuariosValor}>
                    {item.Detalle!.usuarioEntrega ?? '-'}
                  </Text>
                </Text>
                <Text style={styles.detalleUsuarios}>
                  Quien Recibió:{' '}
                  <Text style={styles.detalleUsuariosValor}>
                    {item.Detalle!.usuarioRecibe ?? '-'}
                  </Text>
                </Text>
                ...
                {item.Detalle!.items.map((detalleItem, index) => (
                  <View
                    key={`${detalleItem.folio}-${index}`}
                    style={styles.detalleTableRow}>
                    <Text
                      style={[styles.detalleCell, {flex: 1.3}]}
                      numberOfLines={1}>
                      {detalleItem.folio}
                    </Text>
                    <Text
                      style={[styles.detalleCell, {flex: 1.3}]}
                      numberOfLines={1}>
                      {detalleItem.numeroParte}
                    </Text>
                    <Text style={[styles.detalleCell, {flex: 0.9}]}>
                      {detalleItem.secuencia}
                    </Text>
                    <Text
                      style={[
                        styles.detalleCell,
    ...
    <ImageBackground
      ...
      {loading ? (
        ...
        <FlatList
          ...
          onRefresh={onRefresh}
          ListEmptyComponent={
            <Text style={styles.emptyText}>No hay notificaciones todavía</Text>
          }
        />
      )}
    </ImageBackground>
