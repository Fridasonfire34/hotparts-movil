...
type EntregaProduccionScreenRouteProp = RouteProp<
  RootStackParamList,
  'EntregaProduccion'
>;
...
const EntregaProduccionScreen: React.FC<Props> = ({route}) => {
    ...
    const fetchHotParts = async () => {
      ...
      try {
        const response = await axios.get(
          'http://192.168.16.146:3002/api/hotparts/Produccion',
        );
    ...
    if (rowsWithQuantityOne.length > 0) {
      ...
      try {
        const response = await axios.post(
          'http://192.168.16.146:3002/api/hotparts/cantidadEntrega',
          {
            folios,
            cantidades,
            ordenesCompra,
        ...
        if (response.data.success) {
          ...
          addQrItems(
            rowsWithQuantityOne.map(item => ({
              folio: item.Folio,
              secuencia: item['Secuencia'],
              numeroParte: item['Numero de Parte'],
              cantidad: item['Cantidad Faltante por Entregar'],
            })),
          );
        } else {
  ...
  const enviarListadoEntregaProduccion = async () => {
    ...
    try {
      ...
      // Aviso inmediato a todos los usuarios de que Producción quiere entregar,
      // para que Calidad pueda ir directo a recibir.
      axios
      ...
      console.error('Error al guardar listadoEntregaProduccion:', error);
    }
  };
    ...
    try {
      ...
      if (verifyResponse.data.success) {
        const reciboResponse = await axios.post(
          'http://192.168.16.146:3002/api/hotparts/reciboCalidad',
          {
            folios: foliosSeleccionados,
            nomina: nomina,
          },
        ...
        if (reciboResponse.data.success) {
          ...
          Alert.alert('Éxito', 'Hot Parts Entregados a Calidad', [
            {
              text: 'OK',
              onPress: async () => {
                try {
                  ...
                  const entregaResponse = await axios.post(
                    'http://192.168.16.146:3002/api/hotparts/entregaProduccion',
                    {
                      folios: foliosSeleccionados,
                      nomina: nomina,
                      area: area,
