...
const EntregaProduccionScreen: React.FC<Props> = ({ route }) => {
    ...
    useEffect(() => {
        const fetchHotParts = async () => {
            setLoading(true);
            try {
                const response = await axios.get('http://10.0.2.2:3000/api/Produccion');
    ...
    const handleRecibirHotPart = async () => {
        ...
        if (rowsWithQuantityOne.length > 0) {
            ...
            try {
                const response = await axios.post('http://10.0.2.2:3000/api/cantidadEntrega', {
                    folios,
                    cantidades,
                    ordenesCompra,
                    numerosParte,
    ...
    const handleVerificarCodigos = async () => {
        ...
        try {
            ...
            const verifyResponse = await axios.post('http://10.0.2.2:3000/api/verificarCodigos', {
            ...
            if (verifyResponse.data.success) {
                const reciboResponse = await axios.post('http://10.0.2.2:3000/api/reciboCalidad', {
                    folios: foliosSeleccionados,
                    nomina: nomina,
                });

                if (reciboResponse.data.success) {
                    const guardarMovimientoResponse = await axios.post('http://10.0.2.2:3000/api/guardarMovimiento', {
                        folios: foliosSeleccionados,
                        nomina: nomina,
                    });
                    ...
                    Alert.alert('Éxito', 'Estatus actualizado a Calidad', [
                        {
                            text: 'OK',
                            onPress: async () => {
                                const updateResponse = await axios.get('http://10.0.2.2:3000/api/Produccion');
            ...
            try {
                const eliminarResponse = await axios.post('http://10.0.2.2:3000/api/eliminarCodigos', {});
                        ...
                        {selectedItems.length > 0 && currentItemIndex < selectedItems.length && (
                            <View key={selectedItems[currentItemIndex].Folio}>
                                <Text style={styles.modalTitle}>
                                    El Hot Part: {selectedItems[currentItemIndex]['Numero de Parte']} contiene {selectedItems[currentItemIndex]['Cantidad Faltante por Entregar']} piezas. ¿Cuántas se van a recibir?
                                </Text>
                                <TextInput
                                    style={styles.input}
